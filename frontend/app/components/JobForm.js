...
export default function JobForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/search?page=1`, {
        method: "POST",
        body: formDataObj,
      });
  ...
  const handleSearchAgain = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/search?page=${nextPage}`, {
        method: "POST",
        body: formDataObj,
      });
  ...
  const handlePreviousPage = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/search?page=${prevPage}`, {
        method: "POST",
        body: formDataObj,
      });
